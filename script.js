jQuery(document).ready(function($) {
 ...
 $("#geographicSelectsOptions :input").live('click',function(e){
  ...
  $("#"+currentForm+" :input").each(function(index,value){
   params[$(value).attr("name")]=$(value).val();
  });
  
  imageMarginLeft=Math.round((parseInt($("#"+currentForm+"_div").css('width'))-220)/2);
  $(sectionToReplace).html('<p style=\"color: #5F818A; font-size: 25px; text-align: center; padding-top: '+showStandby_marginTop+'px;\">Standby...</p>');

  params['action']='geographicSelects_AJAX_handler';	

  $(sectionToReplace).load(adminAjaxDir,params, function(response, status, xhr) {
   if (status=="error") $(sectionToReplace).html('Sorry, your browser is having trouble communicating with our server at this time.');
  });
