...
Description: Simple plugin to insert Country and State &lt;select&gt;'s into your website.
Author: Free-SEO.biz
Version: 1.0
...
*/
 ...
 function geographicSelect_insertCountries($atts){
  ...
  foreach ($countryCodes as $index=>$value) $build.="<option value=\"$index\">$value";
  if ($atts['includeselect']) $build.="</select>";
  return $build;
 }
 ...
 class freeseodotbiz_geographicSelect{
  ...
  function optionsPageActual(){
   ...
   echo "     <p><input type=\"radio\" data-whichDIV=\"#geographicSelectForm_countriesOPTGROUP\" name=\"useOptGroup\" value=\"TRUE\">I would like certain countries to show up before all others</p>";
   echo "     <p><input type=\"radio\" data-whichDIV=\"#geographicSelectForm_countriesOPTGROUP\" name=\"useOptGroup\" value=\"FALSE\">Display all countries in order</p>";
   echo "    </form>";
   echo "   </div>";
